...
const addBooks = (request, h) => {
  const id = nanoid(16);
  const {name = '', year = 0, author = '', summary = '', publisher = '',
    pageCount = 0, readPage = 0, reading = false} = request.payload;
  if (name === '') {
    return h.response({status: 'fail', message:
      'Gagal menambahkan buku. Mohon isi nama buku'}).code(400);
  }
  ...
  const response = h.response({status: 'error', message:
    'Buku gagal ditambahkan'}).code(500);
  return response;
};
...
const showAllBooks = (request, h) => {
  ...
  if (books.size === 0) {
    ...
    for (let i = 0; i < books.length; i++) {
      ...
      if (result >= 0) {
        arrBooks.push({id: books[i].id, name: books[i].name,
...
const updateSpecificBook = (request, h) => {
  ...
  if (name === '') {
    return h.response({status: 'fail', message:
      'Gagal memperbarui buku. Mohon isi nama buku'}).code(400);
  } else if (readPage > pageCount) {
    return h.response({status: 'fail', message: 'Gagal memperbarui buku. ' +
      'readPage tidak boleh lebih besar dari pageCount'}).code(400);
  }
  const {bookId} = request.params;
  ...
  if (idx !== -1) {
    ...
    const insertedAt = books[idx].insertedAt;
    books[idx] = {...books[idx], name, year, author, summary,
      publisher, pageCount, readPage, finished, reading, updatedAt, insertedAt};
    return h.response({status: 'success', message:
      'Buku berhasil diperbarui'}).code(200);
  } else {
    return h.response({status: 'fail', message:
      'Gagal memperbarui buku. Id tidak ditemukan'}).code(404);
  }
};
...
const deleteSpecificBook = (request, h) => {
  ...
  if (idx !== -1) {
    books.splice(idx, 1);
    return h.response({status: 'success', message:
      'Buku berhasil dihapus'}).code(200);
  } else {
    return h.response({status: 'fail', message:
      'Buku gagal dihapus. Id tidak ditemukan'}).code(404);
  }
};

module.exports = {addBooks, showAllBooks, showSpecificBook,
  updateSpecificBook, deleteSpecificBook};
